---
name: 220-guest-web-gencode
description: 游客端Web前端代码生成（Nuxt 3 + Vue 3）。当需要为消费者端Web项目生成或更新代码时触发：(1)首次生成TypeScript接口定义和API调用, (2)库表变动后增量更新代码并生成变更报告, (3)备份旧文件供后续裁剪恢复。适用于guest（消费者）角色。
alwaysApply: false
...
从 `PROJECT_ROOT/project-info.md` 读取 `project-name`，扫描对应的 Guest Web 前端项目：

```bash
PN=$(grep 'project-name' project-info.md | head -1 | awk '{print $2}'); ls -1d PROJECT_ROOT/frontend/${PN}-guest-web/
```
...
| 没有前端项目 | 报错，提示先执行 220-guest-web-init |
...
AskUserQuestion({
  questions: [
    {
      question: "要为哪个Guest Web前端项目生成代码？",
      header: "目标前端项目",
      options: [
        { label: "{实际扫描到的项目名}", description: "自动查找可用的Guest Web项目" },
        { label: "手动输入", description: "手动指定Guest Web项目目录名" }
      ],
      multiSelect: false
    },
    {
...
└── PROJECT_ROOT/frontend/
    └── {project-name}-{role}-web/              # 例如: my-shop-guest-web
...
**目录查找规则**：
脚本从 `project-info.md` 读取 `project-name`，查找 `PROJECT_ROOT/frontend/{project-name}-guest-web` 目录（如 `my-shop-guest-web`），即为目标前端项目。
...
代码生成完成后，提示用户进入 **`220-guest-web-dev`** 进行Web端开发（裁剪页面 + 配置路由）。
