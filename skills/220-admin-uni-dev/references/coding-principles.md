...
| 集中到哪里 | 管什么 | 格式 |
|-----------|--------|------|
| `composables/` | 组合函数（权限判断、业务逻辑） | `useXxx()` 函数 |
| `store/` | 全局状态（认证、菜单权限） | Pinia setup 风格 |
| `components/common/` | 通用 UI 组件 | 可复用组件 |
| `utils/` | 工具函数（auth、permission） | 纯函数 |
| `api/` | API 调用封装 | 代码生成器产出，只读不改 |

**具体做法**：
- 页面内**禁止**定义重复的类型接口，应从 `@/api/` 的类型中导入
- 相同的业务逻辑抽取到 `composables/useXxx.ts`
...
```bash
cd frontend/{project-name}-admin-uni
...
### 目录结构
```
src/pages/{role}/{module}/       # 按角色组织页面
src/components/common/         # 通用组件
src/components/business/       # 业务组件
src/composables/               # 组合函数
src/store/                     # Pinia 状态管理
src/api/{module}/index.ts      # API 调用（代码生成器产出，只读不改）
src/utils/                     # 工具函数
pages.json                     # 页面路由配置
```
...
{
  "pages": [
    { "path": "pages/admin/product/list", "style": { "navigationBarTitleText": "商品管理" } },
