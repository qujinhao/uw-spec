...
description: 管理端Web前端代码生成（Vue3 + TS + Vite）。当需要为后台管理Web项目生成或更新代码时触发：(1)首次生成TypeScript接口定义和API调用, (2)库表变动后增量更新代码并生成变更报告, (3)备份旧文件供后续裁剪恢复。适用于root/ops/saas/mch角色。
alwaysApply: false
...
AskUserQuestion({
  questions: [
    {
      question: "要为哪个Admin Web前端项目生成代码？",
      header: "目标前端项目",
      options: [
        { label: "{实际扫描到的项目名}", description: "自动查找可用的Admin Web项目" },
        { label: "手动输入", description: "手动指定Admin Web项目目录名" }
      ],
      multiSelect: false
    },
    {
...
- 选择角色时：构建 `http://{SYSTEM_SERVER}/{project-name}-app/v3/api-docs/{role}Api`
  ...
  - 例如选择 `saas` → `http://{SYSTEM_SERVER}/{project-name}-app/v3/api-docs/saasApi`
...
# 指定前端项目名（多个项目时）
bash scripts/gencode.sh /Users/user/project "http://192.168.88.21/my-shop-app/v3/api-docs/saasApi" "api,page" "my-shop-saas-web"
```
...
└── PROJECT_ROOT/frontend/
    └── {project-name}-{role}-web/              # 例如: my-shop-admin-web
...
代码生成完成后，提示用户进入 **`220-admin-web-dev`** 进行Web端开发（裁剪页面 + 配置路由）。
