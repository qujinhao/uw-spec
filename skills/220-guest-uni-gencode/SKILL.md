---
name: 220-guest-uni-gencode
description: 消费者端UniApp代码生成（UniApp + Vue3）。当需要为消费者端跨平台项目生成或更新代码时触发：(1)首次生成TypeScript接口定义和API调用, (2)库表变动后增量更新代码并生成变更报告, (3)备份旧文件供后续裁剪恢复。适用于guest（消费者）角色。
alwaysApply: false
...
从 `PROJECT_ROOT/project-info.md` 读取 `project-name`，扫描对应的 Guest UniApp 移动端项目：

```bash
PN=$(grep 'project-name' project-info.md | head -1 | awk '{print $2}'); ls -1d PROJECT_ROOT/frontend/${PN}-guest-uni/
```
...
| 没有移动端项目 | 报错，提示先执行 220-guest-uni-init |
...
AskUserQuestion({
  questions: [
    {
      question: "要为哪个Guest UniApp移动端项目生成代码？",
      header: "目标移动端项目",
      options: [
        { label: "{实际扫描到的项目名}", description: "自动查找可用的Guest UniApp项目" },
        { label: "手动输入", description: "手动指定Guest UniApp项目目录名" }
      ],
      multiSelect: false
    },
    {
      ...
      header: "API类型",
      options: [
        { label: "guest", description: "消费者" },
        { label: "open", description: "开放接口" },
        { label: "custom", description: "自定义地址" }
      ],
      multiSelect: false
    },
    {
...
- 选择角色时：构建 `http://{SYSTEM_SERVER}/{project-name}-app/v3/api-docs/{role}Api`
  - 例如选择 `guest` → `http://{SYSTEM_SERVER}/{project-name}-app/v3/api-docs/guestApi`
  - 例如选择 `open` → `http://{SYSTEM_SERVER}/{project-name}-app/v3/api-docs/openApi`
...
# 指定移动端项目名（多个项目时）
bash scripts/gencode.sh /Users/user/project "http://192.168.88.21/my-shop-app/v3/api-docs/openApi" "api,page" "my-shop-guest-uni"
```
...
└── PROJECT_ROOT/frontend/
    └── {project-name}-{role}-uni/           # 例如: my-shop-guest-uni
        └── src/
            ├── api/                      # API 调用模块
            │   └── {module}/             # 按模块子目录
            │       └── index.ts
...
**目录查找规则**：
脚本从 `project-info.md` 读取 `project-name`，查找 `PROJECT_ROOT/frontend/{project-name}-guest-uni` 目录（如 `my-shop-guest-uni`），即为目标前端项目。
...
代码生成完成后，提示用户进入 **`220-guest-uni-dev`** 进行移动端开发（裁剪页面 + 配置路由）。
