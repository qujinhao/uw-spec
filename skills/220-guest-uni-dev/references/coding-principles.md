...
| `mall-widgets/` | 装修组件（`m-*`） | easycom 自动注册 |
...
| UI 组件来源 | `@dcloudio/uni-ui`（`uni-*`）/ `uview-plus`（`u-*`、`up-*`、`u--*`）/ `mall-widgets`（`m-*`） | easycom 自动注册，**禁止重复造同类组件** |
...
| **UI 组件** | 优先使用 easycom 注册的 `uni-*` / `u-*` / `up-*` / `m-*`，避免重复造同类组件 |
...
| **装修组件** | 新增装修能力放 `src/mall-widgets/m-{name}/`，并在 `render-widget.vue` 注册分发；schema 配置放 `src/schema/m-{name}/component.json`；装修组件跳转用 `emit('jump')` 上抛 |
...
| **Loading/空状态** | 页面级遮罩用 `<GbLoading v-model>`；关键提交用 `uni.showLoading + try/finally`；列表上拉/触底用 `<uni-load-more :status>` 三态（more/loading/noMore）；空状态/失败态必备 **图片 + i18n 文案 + 重试按钮**；骨架屏与 GbLoading 二选一；装修 `waiting-cmp` 不用于业务页面 |
...
# 14. 业务页面误用装修预览组件 waiting-cmp（应为 0；仅装修体系内部使用）
grep -rn '<waiting-cmp\|waiting-cmp\.' src/ --include="*.vue" --include="*.ts" | \
  grep -v 'pages/render/' | grep -v 'mall-widgets/' | wc -l

# 15. 自造全屏遮罩 loading（应统一用 GbLoading）
grep -rn 'position:\s*fixed.*z-index:\s*99' src/ --include="*.vue" --include="*.ts" | wc -l

# 16. window/document/navigator 是否在 #ifdef H5 外被引用（应为 0；命中需人工核对是否在 #ifdef H5 包裹内）
grep -rEn '(^|[^.])\b(window|document|navigator)\.' src/ --include="*.vue" --include="*.ts" | wc -l

# 17. plus.* 是否在 #ifdef APP-PLUS 外被引用（应为 0；命中需人工核对）
grep -rn 'plus\.' src/ --include="*.vue" --include="*.ts" | wc -l

# 18. SCSS 错用 // 注释式条件编译（应为 0；CSS 必须用 /* #ifdef */）
