...
if [ ! -f "$INFO_FILE" ]; then
    ...
    echo "  前端项目名    可选，指定具体的前端项目目录名（如 my-shop-guest-uni）"
    echo ""
    echo "示例:"
...
if [ -n "$FRONTEND_PROJECT_NAME_PARAM" ]; then
    ...
    # 查找所有匹配的前端项目目录
    AVAILABLE_FRONTEND_DIRS=$(find "$FRONTEND_DIR" -maxdepth 1 -type d -name "${PROJECT_NAME}-*-uni" 2>/dev/null | sort)

    if [ -z "$AVAILABLE_FRONTEND_DIRS" ]; then
        echo "ERROR: 未找到移动端项目目录: $FRONTEND_DIR/${PROJECT_NAME}-{role}-uni"
        echo "请确保已执行 230-md-uni-init 初始化移动端项目"
        exit 1
    fi
    ...
    if [ "$FRONTEND_DIR_COUNT" -eq 1 ]; then
        ...
        echo "示例: bash scripts/gencode.sh /Users/user/project '' 'api,page' my-shop-guest-uni"
        exit 1
    fi
fi
...
find "$SRC_DIR/api" "$SRC_DIR/router" "$SRC_DIR/pages" "$SRC_DIR/i18n" -type f 2>/dev/null | wc -l | xargs echo "文件总数:"
