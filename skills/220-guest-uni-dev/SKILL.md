...
| 特殊情况（按需） | `src/{module}/` 直接挂 `src` 下 | 当业务方明确不希望路由路径上出现 `packages/` 前缀时（如 `src/product/`、`src/poi/`、`src/user/`） |
...
| 外部 token（H5 嵌入）走 `userStore.setToken(t)`                            | 直接 `uni.setStorageSync('token', t)`       |
...
4. 都没有 → 在 src/components/ 新建（命名遵循 Z* / Gb* / 业务名）
```
...
**TabBar 是项目固定配置项**。无论 PRD 是否明确要求，所有移动端项目都必须按本约定在 [pages.json](../../src/pages.json) 中配置 `tabBar`。

| ✅ 正确                                                                                                    | ❌ 错误                               |
| ---------------------------------------------------------------------------------------------------------- | ------------------------------------- |
| 在 [pages.json](../../src/pages.json) `tabBar.list` 中配置 Tab 列表（常见 3~5 个）                         | 不配置 `tabBar`，省略 Tab 体系        |
| Tab 页面切换使用 `uni.switchTab()`                                                                         | Tab 页面使用 `uni.navigateTo()`       |
| Tab 对应页面必须同时在 `pages` 中以**主包**形式注册（不能放分包）                                          | TabBar 页面放进分包                   |
| TabBar 图标使用 `static/tabbar/` 目录（普通态 / 选中态分文件）                                             | 图标放 `src/assets/` 或与业务图片混用 |
| TabBar 颜色（`color` / `selectedColor`）使用主题变量统一管理                                               | 在 `pages.json` 里硬编码十六进制      |
| TabBar 文案走 i18n（通过 [main store](../../src/store/main.ts) 的 `setI18n()` 设置 `tabBar.list[i].text`） | TabBar `text` 硬编码中文              |
...
| TabBar 配置           | "确认 TabBar Tab 数量与对应页面（项目固定配置 TabBar）" |
...
| TabBar   | 在 `tabBar.list` 中配置 Tab，主包注册对应页面（项目固定配置 TabBar）                                            |
...
- [ ] pages.json 路由配置正确；TabBar 在 `tabBar.list` 中按 PRD 完整配置
- [ ] H5 和微信小程序编译通过
- [ ] `pnpm check`（type-check + oxlint + oxfmt）通过
- [ ] 业务文案统一走 `vue-i18n`（`$t()` / `t()`），4 种语言（zh-CN / zh-TW / en / ja）的 key 同步补齐
- [ ] UI 组件优先复用 `@dcloudio/uni-ui`（`uni-*`）/ `uview-plus`（`u-*` / `up-*`），避免自造重复组件
